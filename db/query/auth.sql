...
SELECT authentications.*, user_roles.role_id, entity_profiles.*, accounts.total_coin
FROM authentications
JOIN user_roles ON authentications.id = user_roles.user_id
JOIN accounts ON authentications.id = accounts.user_id
LEFT JOIN entity_profiles ON authentications.id = entity_profiles.user_id
-- LEFT JOIN users ON authentications.id = users.user_id AND authentications.user_type = 'user'
-- LEFT JOIN churches ON authentications.id = churches.auth_id AND authentications.user_type = 'churchAdmin'
...
-- name: MentionExists :one
SELECT EXISTS(
    SELECT 1 FROM post_mentions WHERE post_id = $1 AND mentioned_user_id = $2
);

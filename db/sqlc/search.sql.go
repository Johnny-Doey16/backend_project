...
type GetSearchResultRow struct {
	Source string          `json:"source"`
	Data   json.RawMessage `json:"data"`
}

func (q *Queries) GetSearchResult(ctx context.Context, arg GetSearchResultParams) ([]GetSearchResultRow, error) {
	rows, err := q.db.QueryContext(ctx, getSearchResult,
		arg.PlaintoTsquery,
		arg.FollowerUserID,
		arg.Limit,
		arg.Offset,
	...
	defer rows.Close()
	items := []GetSearchResultRow{}
	for rows.Next() {
		var i GetSearchResultRow
...
const getSearchResultOldNoFTS = `-- name: GetSearchResultOldNoFTS :many
...
type GetSearchResultOldNoFTSParams struct {
...
type GetSearchResultOldNoFTSRow struct {
	Source string          `json:"source"`
	Data   json.RawMessage `json:"data"`
}

// TODO: Limit and offset for users, church, posts individually rather than for all.
func (q *Queries) GetSearchResultOldNoFTS(ctx context.Context, arg GetSearchResultOldNoFTSParams) ([]GetSearchResultOldNoFTSRow, error) {
	rows, err := q.db.QueryContext(ctx, getSearchResultOldNoFTS,
		arg.Column1,
		arg.FollowerUserID,
		arg.Limit,
		arg.Offset,
	...
	defer rows.Close()
	items := []GetSearchResultOldNoFTSRow{}
	for rows.Next() {
		var i GetSearchResultOldNoFTSRow
		if err := rows.Scan(&i.Source, &i.Data); err != nil {
			return nil, err
		}
		items = append(items, i)

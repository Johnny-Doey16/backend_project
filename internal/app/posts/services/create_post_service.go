...
func GetCreatePostStreamData(stream pb.SocialMedia_CreatePostServer, imgStore services.ImageStore) ([]string, [maxImages]string, string, error) {
	...
	for {
		...
		case *pb.CreatePostRequest_Content:
			...
			if err := checkBody(content); err != nil {
				log.Println("Check body err", err)
...
func checkBody(content string) error {
	if content == "" {
...
func CreatePostInDB(ctx context.Context, td *worker.TaskDistributor, db *sql.DB, store *sqlc.Store, uid uuid.UUID, content, author string, imageUrls []string, captions [maxImages]string) (uuid.UUID, sqlc.Post, error) {
	...
	sqlPost, err := runConcurrentPostCreationTasks(td, tx, qtx, ctx, postID, imageUrls, captions, content, author, uid)
	if err != nil {
...
// TODO: Use err.Group and test
func runConcurrentPostCreationTasks(td *worker.TaskDistributor, tx *sql.Tx, qtx *sqlc.Queries, ctx context.Context,
	postId uuid.UUID, images []string, captions [maxImages]string, p_content, author string,
	...
	createHashtagChan := make(chan error, 1)
	...
	// ! 3 create posts Mentions if there is any
	wg.Add(1)
	go func() {
		defer wg.Done()
		err := DetectMentions(ctx, td, author, p_content, qtx, tx, uid, postId)
	...
	go func() {
		defer wg.Done()
		err := ProcessHashtags(ctx, p_content, tx, postId)
		createHashtagChan <- err
	}()
	...
	if err := <-createHashtagChan; err != nil {
		tx.Rollback()
		return sqlc.Post{}, errors.New("an unknown error occurred creating hashtag " + err.Error())
	}
